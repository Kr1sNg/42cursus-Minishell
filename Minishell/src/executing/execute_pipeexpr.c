...
/*   Updated: 2025/03/14 17:17:51 by tat-nguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exec_left_child(t_ast_pipeexpr *ast, t_env *env,
			int fd[2], int *status)
{
	if (ast->left)
	{
...
static void	exec_right_child(t_ast_pipeexpr *ast, t_env *env,
			int fd[2], int *status)
{
	if (ast->right)
	{
...
int	ft_exe_pipeexpr(t_ast_pipeexpr *ast, t_env *env)
{
	...
	if (pid_left == -1)
		return (status);
	if (pid_left == 0)
		exec_left_child(ast, env, fd, &status);
	pid_right = fork();
	if (pid_right == -1)
		return (status);
	if (pid_right == 0)
